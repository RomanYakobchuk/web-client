...
interface IProps {
    ...
    numberOfColumnsByWidth?: {xs?: string, sm?: string, md?: string, lg?: string} | string
}

const PropertiesList = ({items, setIsOpen, numberOfColumnsByWidth = undefined}: IProps) => {

    const {variantShowItems} = useContext(VariantContext);

    const defaultNumberOfColumnsByWidth = 'repeat(auto-fit, minmax(175px, 1fr))';

    const style: SxProps = variantShowItems?.establishment === 'variant_1'
        ? {
            display: 'grid',
            gridTemplateColumns: numberOfColumnsByWidth ? numberOfColumnsByWidth : defaultNumberOfColumnsByWidth,
