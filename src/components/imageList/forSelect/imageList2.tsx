...
const ImageList2 = ({items, rowHeight, maxLength, addImage, deleteImage, maxWidth, type}: IProps) => {
    ...
    return (
        ...
        }} cols={items?.length < 3 ? items?.length : 3} rowHeight={rowHeight}>
            {items?.map((image: any, index: number) => (
                    <ImageListItem key={index} sx={{
                        ...
                        <ImageField preview={{zIndex: 10000, width: (device || width < 600) ? '90%' : 'auto'}}
                                    value={type === 'show' ? image.url : image instanceof Blob ? URL.createObjectURL(image) : image?.url} style={{
                            objectFit: 'cover',
                            height: '100%',
                            width: '100%',
                            borderRadius: '5px',
